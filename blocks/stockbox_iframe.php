...
while ( list(,$Sym) = each($Symbols) ) {
	...
	if (! $quote->get_stock_quote($Sym)) {
		$quotes[$i] = "<table style='padding: 0; margin: 0; font-size: ". $fontsize . "px;'><tr><td><strong>" . $quote->symbol . "</strong></td> ";
		$quotes[$i] .= "<td>" . $quote->change . "</td>";
		$quotes[$i] .= "<tr><td><i>" . _SB_DATE . "</i></td><td>" . $quote->date . "</td></tr>";
		$quotes[$i] .= "<tr><td><i>" . _SB_TIME . "</i></td><td>" . $quote->time . "</td></tr>";
		$quotes[$i] .= "<tr><td><i>" . _SB_LAST . "</i></td><td>" . $quote->last . "</td></tr>";
		$quotes[$i] .= "<tr><td><i>" . _SB_HIGH . "</i></td><td>" . $quote->high . "</td></tr>";
		$quotes[$i] .= "<tr><td><i>" . _SB_LOW . "</i></td><td>" . $quote->low . "</td></tr>";
		$quotes[$i] .= "<tr><td><i>" . _SB_PREV . "</i></td><td>" . $quote->open . "</td></tr>";
		$quotes[$i] .= "<tr><td><i>" . _SB_VOLUME . "</i></td><td>" . $quote->volume . "</td></tr>";
		$i++;
	}
}
...
if ($xoopsUser) {
	echo '<tr colspan="2"><td><center><a target="_parent" href="'.XOOPS_URL.'/modules/stockquotes/stockbox_custom.php?default='.$default_stocks.'">'._SB_CUSTOMIZE.'</a></center></td></tr></table>';
}
